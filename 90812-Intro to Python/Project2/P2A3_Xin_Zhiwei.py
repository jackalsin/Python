...
def main():
    ...
    # calculate the precentage of the uniqueness
    THRESHOLD = 0.1
    uniqueRatio = calculatePercentage(oldUniquePatchList, newUniquePatchList)
    print("The unique word percentage in new patch = %.5s%% " % (uniqueRatio * 100))
    print("Assume threshold = ", THRESHOLD)
    # print the result
    if uniqueRatio > THRESHOLD:
        print("A senior or junior analyst must handle the patch")
    else:
        print("It's unnecessary to let a senior or junior analyst must handle the patch")


def calculatePercentage(oldPatchList, newPatchList):
    oldPatchSet = set(oldPatchList)
    unique = 0
    total = 0

    for word in newPatchList:
        if (word not in oldPatchSet):
            unique += 1
        total += 1
