...
class Game(object):
    ...
    def __init__(self, mode,saved = None, modified = False):
        ...
        if saved == None:
            ...
            if self.modified == False:
                ...
                # self.superBoard.printMap() # testCode
    ...
    def drawBoard(self):
        ...
        for row in range(rows):
            for col in range(cols):
                ...
                # pos = self.superBoard.board[row][col].image.get_rect().move((x,y))
                # print(type(image))

                # print(self.superBoard.board[row][col].image)
    ...
    def initAI(self):
        AIBirthCor = (680,40)
        self.user[1] = player.Player(2,AIBirthCor,"players")
        self.players.add(self.user[1])
        self.user[1].lives = 0
        ...
        while not self.done:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.QUIT:
                    ...
                    if self.mode == self.c.SINGLE:
                        if k == pygame.K_SPACE:
                            print("player 1 deploying a bomb")
                            ...
                            print("player 1 moving!! in game.py")
                        ...
                        if self.hasAI == True:
                            AIPath = self.findAWay(self.user[1].position,
                                                    self.user[0].position)

                            nextKey = self.getKey(AIPath[0])
                            print("nextKey",nextKey)
                            squareOffset = self.user[1].moveSquare(nextKey)
                            print("squareOffset = ", squareOffset)
                            # [above]in Char, return a tuple
                            canMove = self.moveCheck(self.user[1], squareOffset)
                            # [above] input is x y cordination

                            # if canMove == False:
                            #     x = squareOffset[0] * (-1)
                            #     y = squareOffset[1] * (-1)
                            #     self.moveCheck(self.user[1], (x,y))
                ...
                # for users interact with menu
                    ...
                    # init AI
                                ...
                                2*rMenuHeight, self.c.WIDTH,3 * rMenuHeight):
                        self.hasAI = True
                        self.initAI()
                    # load Map
                    elif self.inBoundary(pos[0],pos[1],rMenuStartX, 
    ...
    def avoidBomb(self):
        dest = self.generateSafe()
        print("dest = ",dest)
        AIPath = self.findAWay(self.user[1].position, dest)
        nextKey = self.getKey(AIPath[0])
        print("nextKey = ", nextKey)
        squareOffset = self.user[1].moveSquare(nextKey)
        print("squareOffset = ",squareOffset)
    ...
    def generateSafe(self):
        ...
        for b in self.bombsGroup:
            bRow = b.position.y // self.c.TILE_SIZE
            bCol = b.position.x // self.c.TILE_SIZE
            bRange = b.range
            for row in range(bRow - bRange, bRow + bRange + 1):
                if 0 < row < len(self.superBoard.map): 
                    for col in range(bCol - bRange, bCol + bRange + 1):
                        if 0 < col < len(self.superBoard.map[0]):
                            forbidden.append((row, col))
        while True:
            row = random.randint(1, len(self.superBoard.map) - 1)
            col = random.randint(1,len(self.superBoard.map[0]) - 1)
    ...
    def getKey(self, aTuple):
        print("getKey")
        # here should be upside down
        if aTuple == (-1,0):
    ...
    # map, otherPos
    def findAWay(self,pos, otherPos):
        print("enter findAWay")
        self.direction = [None,None]
        self.steps = [None,None]
        if type(otherPos) == tuple:
            self.steps[0] = otherPos[0]
            self.steps[1] = otherPos[1]
            dx = self.steps[0] * self.c.TILE_SIZE
            dy = self.steps[1] * self.c.TILE_SIZE
            dList = [dx, dy]
        else:
            dx = - pos.x + otherPos.x
            dy = - pos.y + otherPos.y
            dList = [dx, dy]
            self.steps[0] = dx//self.c.TILE_SIZE
            self.steps[1] = dy//self.c.TILE_SIZE
        ...
        if dx == 0 or dy == 0:
            return [self.moveLine(self.superBoard.map)]
        else:
            return [self.moveTriangle(self.superBoard.map)]

    def moveLine(self, thismap):
        print("moveline")
        self.row = self.user[1].position.y//self.c.TILE_SIZE
        self.col = self.user[1].position.x// self.c.TILE_SIZE
        row = self.row
        col = self.col
    ...
    def moveTriangle(self,thismap):
        print("moveTriangle")
        allPossible = []
        initMove = self.initMoveGen(self.steps)
        print(initMove)
        index = random.randint(0, len(initMove) -1)
        return initMove[index]
       

    def initMoveGen(self,steps):
        print("initMoveGen")
        result = []
        for i in range(abs(steps[0])):
            if steps[0] < 0:
                result.append((-1,0))
            else:
                result.append((1,0))
    ...
    def getPer(self, a):
        ...
        if (len(a) == 0):
            ...
            allPerms = [ ]
            for subPermutation in self.getPer(a[1:]):
    ...
    def saveScore(self, name, score):
        print("in game.py enter saveScore")
