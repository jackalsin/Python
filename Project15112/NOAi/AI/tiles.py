...
class Tile(pygame.sprite.Sprite):
    ...
    def initAttributes(self):
        ...
        # if self.type in [self.c.BOMB_UP,self.c.POWER_UP,
        #                         self.c.LIFE_UP,self.c.TIME_UP,self.c.DOOR]:
        #             print("load self.type = %d" % (self.type))

        # the line right above is cited from the package owner 
        # print("self.path = ", self.path, self.type)
        self.image = pygame.image.load(self.path)

    def destroy(self):
        # print("enter destroy, the type = ",self.type)
        if self.type == self.c.BRICK:
        ...
        # the following logic:
            ...
            if self.brickDown == False:
                ...
                self.image = pygame.image.load(self.path)
                self.passable = True
                print("set passable ", self.passable)
            else:
