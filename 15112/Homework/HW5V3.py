...
import copy
def noAlphabet(twoDList):
    (rows,cols)=(len(twoDList),len(twoDList[0]))
    for row in range (rows):
        for col in range(cols):
            if type(twoDList[row][col]) != int:
                print("now checking twoDList[row]col",twoDList[row][col])
                return False
    return True

def isMagicSquare(a):
    if (len(a)==1 and type(a[0])==int): return False
    if (isEmpty(a) or ((isRag(a))==False) or (len(a)!=len(a[0])) 
        or (noDuplicateElem(a)==False)): 
        return False
    else:
        if noAlphabet(a) == False: return False
...
def noDuplicateValueIn1dList(aList):
    for index in range(len(aList)):
        if aList.count(aList[index])>1:
            return False
    return True

def returnListWithoutZero(aList):
    noZeroValues=[]
    for index in range(len(aList)):
        if aList[index]!=0:
            noZeroValues.append(aList[index])
    return noZeroValues
############### Question 3 ############
...
def isSquare(board): # ensure it's a square 
...
def searchLowestCostPositionInDirection(board,word,startRow,startCol,direction):
    ...
    for i in range(len(word)):
        ...
        if ((row < 0) or (row >= rows) or (col < 0) or (col >= cols)):
            ...
            if (board[row][col] == word[i]): # already fixed
                continue
            elif (board[row][col] == '-'):
                cost+=1 # if Not ALlow, use cost == 0
            else:
                return None
...
def replaceDash(board,startRow,startCol):
    ...
    for dir in range(8):
        ...
        if ((row >= 0) and (row < rows) and
            ...
            (board[row][col] in alphabet)):
            alphabet.remove(board[row][col])
...
def testIsRag():
    ...
    assert(isRag([[8, 1, 6], [3, 5, 7], [4, 9]])==False)
...
def testIsMagicSquare():
    ...
    assert(isMagicSquare(([['EIGHT', 1, 6], [3, 5, 7], [4, 9, 2]]))==False)
...
def testIsKingTour():
    ...
    A4=[[  1, 14, 15, 16],
        ...
        [ 12,  8,  3,  5],
        [ 11, 10,  9,  4]]
    assert(isKingsTour(A1) == True)
    assert(isKingsTour(A2) == False)
    assert(isKingsTour(A3) == False)
    assert(isKingsTour(A4) == True)
    print("Passed. (Add more tests to be more sure!)")
...
def testNoAlphabet():
    print("Testing noAlphabet()...", end="")
    assert(noAlphabet([[1,2,3],[1,2,3],[1,2,3]]) == True)
    assert(noAlphabet([[1,2,3],[1,3,2],[1,2,3]]) == True)
    assert(noAlphabet([[1,2,3],[1,2,"3"]]) == False)
    print("Passed. (Add more tests to be more sure!)")

def testReturnListWithoutZero():
    print("Testing returnListWithoutZero()...", end="")
    assert(returnListWithoutZero([1,2,3,1,2,3,1,2,3]) == [1,2,3,1,2,3,1,2,3])
    assert(returnListWithoutZero([1,2,3,1,2,3,1,2,0]) == [1,2,3,1,2,3,1,2])
    assert(returnListWithoutZero([0]) == [])
    print("Passed. (Add more tests to be more sure!)")
