...
class Gate(object):
    ...
    def saveSelf(self,data):
        toWrite = (str(self.gateType)+' '+str(self.maxInputGates)+' '+
            str(self.maxOutputGates)+' '+str(self.x)+' '+str(self.y)+' '+ '\n')
        with open(data.GatesFileName,"a") as f:
            f.write(toWrite)

    def loadSelf(self,data,content):
        valueList =[]
        for item in content.split(" "):
            valueList.append(item)
        self.maxInputGates = int(valueList[1])
        self.maxOutputGates = int(valueList[2])
        self.x = float(valueList[3])
        self.y = float(valueList[4])
...
class Wire(object):
    ...
    def saveSelf(self,data):
        toWrite = (str(self.startX)+' '+str(self.startY)+' '+str(self.endX)+ ' '
           +str(self.endY)+' '+str(self.status)+' '+str(self.startIndex)+' '+ '\n')
        with open(data.WiresListName,"a") as f:
            f.write(toWrite)
...
def init(data):
    ...
    data.GatesFileName = "gates.txt"
    data.WiresFileName = "wires.txt"
    data.WiresListName = "wireslist.txt"
    data.connectInfoList = []
    printInstruction()
    pass

def printInstruction():
    print(   '''1. You can only modify the circuit when power is off.
   Once on, you cannot modify it
2. Please connect the components by click the input/output pin, 
   rather than the component. And first click the output pin, 
   then the input pin''')
    ...
    # effective canvas
        ...
        if type(data.drawType)==int:
            ...
            if None not in [data.wireX1,data.wireY2]: # value set,create obj
                ...
                data.connectInfoList.append((data.startIndex,data.endIndex))
...
def do_save(data):
    # save Gates
    open(data.GatesFileName,"w").close()
    for elem in data.components:
        elem.saveSelf(data)

    print(data.connectInfoList)
    open(data.WiresFileName,'w').close()

    with open(data.WiresFileName, "a") as f:
        for i in range(len(data.connectInfoList)):
            content = ''
            for j in range(len(data.connectInfoList[i])):
                content +=str(data.connectInfoList[i][j])+' '
            content += '\n'
            f.write(content)
    f.close()
    open(data.WiresListName,"w").close()
    for elem in data.wires:
        elem.saveSelf(data)

    print("we finish saving")


def do_load(data):
    do_clear(data)
    with open(data.GatesFileName,"rt") as f: # load Gates
        content = f.readlines()
    for i in range(len(content)):
        gateType = content[i].split(" ")[0]
        temp = Gate(gateType)
        temp.loadSelf(data,content[i])
        data.components.append(temp)
    f.close()

    with open(data.WiresFileName,"rt") as f: # load connection
        content = f.readlines()
    for i in range(len(content)):
        line = content[i]
        connectList=line.split(" ")
        startIndex = int(connectList[0])
        endIndex = int(connectList[1])
    ...
    # loadWire
    with open(data.WiresListName,"rt") as f: # load Gates
        content = f.readlines()
    for i in range(len(content)): 
    # startIndex,startX,startY,endX,endY
        tempList = content[i].split(" ")
        startX = float( tempList[0])
        startY = float(tempList[1])
        endX = float(tempList[2])
        endY = float(tempList[3])
        startIndex = int(tempList[5])
        tempWire = Wire(startIndex,startX,startY,endX,endY)
        boolW=tempList[4]
        if boolW == 'True':
            status = True
        else:
            status = False
        tempWire.loadSelf(data,status)
        data.wires.append(tempWire)

# (str(self.startX)+' '+str(self.startY)+' '+str(self.endX)+ ' '            +str(self.endY)+' '+str(self.status)+' '+str(self.startIndex)+' '+ '\n')
