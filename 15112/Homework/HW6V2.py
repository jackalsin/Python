...
O(n) while loop will go through every element in b 

def slow1(a):
    return len(a)

it's O(1). Function len(a) is O(1)


slow2:
to check if there is identical elements
it's O(n^2) the nested loop causes the O(n^2)
def slow2(a):
    aSet=set(a)
    return len(aSet) == len(a)

it's O(1). Er... the O(1) comes from len()
...
def testFriendsOfFriends():
    ...
    assert(friendsOfFriends({'barney': set(), 
                        'betty': set(), 'bam-bam': set(), 
                        'wilma': {'betty', 'fred', 'dino'}, 
                        'fred': {'barney', 'wilma', 'betty', 'bam-bam'}, 
                        'dino': set()}) == {
                        'barney': set(), 
                        'betty': set(),
                        'bam-bam': set(), 
