...
def make2DList(elem,rows,cols):
    result = []
    for row in range(rows):
        result.append([0] * cols)
    return result 

def initIntPos(board):
    result = []
...
def isLegal(rectBoard,row0,col0,width,height):
    (rows,cols) = (len(rectBoard),len(rectBoard[0]))
    if (row0<0 or col0<0 or ((row0 + height-1)>=rows) or ((col0 + width-1)>=cols)):
        return False
    for drow in range(height):
        for dcol in range(width):
            (curRow, curCol) = (row0 + drow, col0 + dcol)
            if rectBoard[curRow][curCol] != 0:
                return False
    return True

def setBoard(board,row0,col0,width,height):
    for drow in range(height):
        for dcol in range(width):
            (curRow, curCol) = (row0 + drow, col0 + dcol)
...
def removeBoard(board,row0,col0,width,height):
    for drow in range(height):
        for dcol in range(width):
            (curRow, curCol) = (row0 + drow, col0 + dcol)
...
def solveRectangula(board):
    ...
    intPositions = initIntPos(board) # (row, col, intValue), 
    rectBoard = make2DList(0,rows,cols) # 2d list all zero
    solution = [] # (row0, col0, width, height)
    def solve(intPositionsIndex):
        if (intPositionsIndex == len(intPositions)):
            return solution
        else:
            (intRow,intCol,intValue) = intPositions[intPositionsIndex]
            shapes = areaToForms(intValue)
            for (width,height) in shapes:
                (startRow, startCol) = (intRow-height+1,intCol-width+1)
                for drow in range(height):
                    for dcol in range(width):
                        (row0,col0) = (startRow+drow,startCol+dcol)
                        if isLegal(rectBoard,row0,col0,width,height):
                            solution.append((row0,col0,width,height))
                            setBoard(rectBoard,row0,col0,width,height) # return?
                            result = solve(intPositionsIndex + 1)
                            if result != None:
                                return result
                            solution.pop()
                            removeBoard(rectBoard,row0,col0,width,height) # return
...
class Boss(Monster):
    def __init__(self,x,y,row,col,data):
        ...
        self.r = 2 * data.MonsterSize
        self.monsterColor = "spring green"
...
#   # canvas
    ...
    data.boss = Boss(data.width/2,250,0,0,data) # boss
...
def chooseMonsterFire(data):
    ...
    bossFire = random.randint(0,100)
    if bossFire > 88 and data.boss != None:
        bossBullet = MonsterBullet(data.boss.x,data.boss.y)
        data.monsterBullets.append(bossBullet)
...
def moveMonster(data,stepLength):
    ...
    if data.boss != None:
        data.boss.move(stepLength * random.randint(0,20)/10)
...
def checkMonster(data, x,y):
    ...
    if data.boss != None:
        if distance(x,y,data.boss.x,data.boss.y) <= data.boss.r:
            data.boss = None
    return False
...
class OopyInvaders(object):
    ...
    def __init__(self):
        ...
        self.data.timerDelay = 100 # milliseconds
        init(self.data)
