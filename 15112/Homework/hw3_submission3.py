...
def patternedMessage(message,patterned):
    ...
    # check every charater in patterned and copy to another
        if (patterned[i]=='\n' and 
            (i==0 or i==(len(patterned)-1))):
            continue
        elif isAWhitespace(patterned[i]):
            result+=' '
        elif (patterned[i]=='\n'):# start a new line
            result+='\n'
...
def getCols(encodeLetters,rows):
    cols=round(len(encodeLetters)/rows)
    return cols

# print(getCols("WCWNKEAAzyTTTDxwAA",6))

def tranform(encodeLetters,rows,cols): # make the odd line reversed
    result=""
    for i in range (rows):
        temp=encodeLetters[cols*i:(i+1)*cols]
        # print("i and temp", i , temp)
        if i%2==1:
            temp=reverseString(temp)
        # print("afterReverse", temp)
        result+=temp
    return result

# print(tranform("WCWNKEAAzyTTTDxwAA",6,3))
def validChar(temp):
...
def decodeRightLeftCipher(encodedMessage):
    ...
    for c in encodedMessage:
        if c.isdigit():
            rowsInString+=c
    rows=string2Digit(rowsInString) 
    encodeLetters=encodedMessage[len(rowsInString):]
    # print("cols and enco""deLetters", cols, encodeLetters)
    result=""
    temp=""
    cols=getCols(encodeLetters,rows)
    initialString=tranform(encodeLetters,rows,cols)
    for jCol in range (cols):
        for iRow in range (rows):
            temp=initialString[cols*iRow+jCol]
            if validChar(temp):
                result+=temp
    return result

# print(decodeRightLeftCipher('2LTCMVOTPIJWZGRYUGOSUIXUCRJCZNC'))
...
def testPatternedMessage():
    ...
    assert(patternedMessage('12345','\n## ### ####### ###\n') 
        == '12 345 1234512 345') 
    assert(patternedMessage('A', 'x y z')=='A A A')
...
def testDecodeRightLeftCipher():
    ...
    assert(decodeRightLeftCipher('6WCWNKEAAzyTTTDxwAA') ==  "WEATTACKATDAWN")
    assert(decodeRightLeftCipher("2LTCMVOTPIJWZGRYUGOSUIXUCRJCZNC")=="LCTNCZMCVJORTCPUIXJIWUZSGORGYU")
    assert(decodeRightLeftCipher('10MCWJSIRNLZXEHOANWJVRUNBDTHBMDZ')=='MIREHJVDTZCSNXOWRBHDWJLZANUNBM')
    print("Passed. (Add more tests to be more sure!)")
