...
def getCourse(courseCatalog, courseNumber):
    print()
    visited.append(courseCatalog[0])
    print("we are checking",visited, "cata = ", courseCatalog)
    L = copy.deepcopy(courseCatalog)
    if (type(L[1]) != list):
        print("enter if ")
        if courseNumber in L:
            print("in courseNumber ",visited, "courseNumber = ",courseNumber)
            visited.append(courseNumber)
            print("we did some", visited)
            return True
        else:
            # visited.pop(-1)
            return False
        ...
        if (index == len(L)):
            ...
            visited.remove(str(L[index-1][0]))
            return False
        else:
            if(getCourse(L[index],courseNumber) == False):
                print("before move visited = ",visited,"elem = ",L[index][0])
                visited.remove(str(L[index][0]))
                print("visited = ", visited)
                index += 1
                # visited.append(str(L[index][0]))
                return getCourse(L[index],courseNumber)
            else:
                # print(visited)
                return True
                # return True
                # make visited in to string
def testGetCourse():
    print("getCourse()...")#,end = "")
    courseCatalog = ["CMU",
                        ["CIT",
    ...
    getCourse(courseCatalog, "15-112")
    print(visited)
    print("passed")
    pass
